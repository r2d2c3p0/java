...
public class GetVMArguments {
	...
	private final static Logger logger = Logger.getLogger(GetVMArguments.class);
	...
	*/
	public static void run() {
		...
		List<String> jvmArguments = mbean.getInputArguments();
		for (int iargs = 0; iargs < jvmArguments.size(); iargs++) {
			logger.info(" jvm argument" +iargs+ " "+jvmArguments.get(iargs));
		}
		logger.info(" -classpath :" + System.getProperty("java.class.path"));
		logger.info(" executing :" + System.getProperty("sun.java.command"));
		logger.debug(" end");
	}
